...
echo -e "IP Info: $(ip -4 addr | grep inet | tail -2  | head -1 | awk '{print $2}') \n" >> ~/research/sys_info.txt
echo -e "Hostname: $(hostname -s) \n" >> ~/research/sys_info.txt
echo "DNS Servers: " >> ~/research/sys_info.txt
cat /etc/resolv.conf | tail -2  >> ~/research/sys_info.txt
...
df -H | sort -r -k 5 -i | head -2 >> ~/research/sys_info.txt
echo -e "\nWho is logged in: \n $(who -a) \n" >> ~/research/sys_info.txt
echo -e "\nSUID Files:" >> ~/research/sys_info.txt
find / -type f -perm /4000 2>/dev/null >> ~/research/sys_info.txt
